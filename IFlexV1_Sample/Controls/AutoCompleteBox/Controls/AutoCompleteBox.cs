...
namespace System.Windows.Controls
{
    ...
    public partial class AutoCompleteBox : Control, IUpdateVisualState
    {
        ...
        public static readonly DependencyProperty SearchKindProperty =
            DependencyProperty.Register(
            "SearchKind",
            typeof(InstantSearchKind),
            typeof(AutoCompleteBox));

        public InstantSearchKind SearchKind
        {
            get
            {
                return (InstantSearchKind)GetValue(SearchKindProperty);
            }
            set
            {
                SetValue(SearchKindProperty, value);
            }
        }
        ...
        private void OnSelectedItemChanged(object newItem)
        {
            ...
            {
                InstantSearch instant = new InstantSearch(SearchText);
                if (instant.HasMember)
                {
                    instant.Member[instant.Member.Length - 1] = FormatValue(newItem, true);
                }
                else
                    instant.Criteria[instant.Criteria.Length - 1] = FormatValue(newItem, true);

                text = instant.SearchText;
            }
