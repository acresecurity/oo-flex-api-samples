...
AnsiConsole.Write(new Panel(info).Header("Hardware"));
    ...
    .ConfigureServices((context, services) =>
    {
        ...
        services.AddSingleton(provider =>
        {
            ...
            app.Configure(config =>
            {
                ...
                config.AddBranch<HardwareSettings>("door", p =>
                {
                    p.AddCommand<MomentarilyUnlockDoorCommand>("momentary")
                        .WithExample(new[] { "door", "momentary", Guid.NewGuid().ToString() });

                    p.AddCommand<DoorModeCommand>("mode")
                        .WithExample(new[] { "door", "mode", Guid.NewGuid().ToString(), "Unlocked"});
                });
            });
            return app;
        });
